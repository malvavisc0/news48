...
def test_compose_includes_base_prompt():
    ...
    assert "Make the minimum calls needed" in result
...
def test_parser_business_logic_matches_caller_verification_model():
    """Parser business logic includes post-write review and self-verification."""
    content = Path(
    ...
    assert "articles info" in verify_skill
    assert "Re-read the staged file" in review_skill
    ...
    for agent in ["executor", "parser", "fact_checker", "sentinel"]:
        ...
        if base_path.exists():
            ...
            assert estimated_tokens < 1300, (
                ...
                f"~{estimated_tokens:.0f} tokens (expected < 1300)"
            )
