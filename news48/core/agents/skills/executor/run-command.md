...
1. Pass `timeout` as a **tool parameter**, not a CLI flag.
   - Correct: `run_shell_command(command='uv run news48 download ...', timeout=300)`
   - Wrong: `run_shell_command(command='uv run news48 download ... --timeout=300')`
2. Timeout guidance:
...
4. Always run CLI commands as `uv run news48 ...` and pass `--json` whenever the command supports it.
5. Use `--feed` for per-domain steps; omit when plan scope requires broader execution.
6. When `uv run news48 download` is used for feed-scoped backlog work, pass `--limit` explicitly when needed and repeat the command until the plan outcome is reached, no eligible backlog remains, or progress stalls.
