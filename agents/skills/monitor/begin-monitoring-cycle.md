...
If denominator is 0, rate is **undefined** — record as `null` and note "insufficient sample". Do not treat 0/0 as 0% and do not extrapolate.

### Step 3: Compare Against Thresholds
Compare each computed metric against the canonical threshold table. Do not duplicate threshold values here — the table is the single source of truth.
...
Write the report to `.monitor/latest-report.json` using Python `json.dump`. Include keys: `timestamp` (ISO 8601 UTC), `status`, `metrics`, `alerts`, `recommendations`. This file is read by the Planner at the start of each planning cycle.

### Step 7: Persist Metrics History
Write a timestamped metrics file to `.metrics/<ISO-timestamp>.json` with the cycle's computed metrics. This enables trend analysis across cycles.
...
- If a branch condition is discovered mid-cycle (breach found, alerts needed, fact-check drift found), handle it during the current run — do not defer to the next cycle.
